...
class VideoPlayerPip {
  ...
  /// Enables auto-enter Picture-in-Picture mode for the given video player controller.
  ///
  /// This allows the PiP mode to be automatically triggered when the user leaves the app (backgrounds it),
  /// provided that the video is playing.
  ///
  /// Returns a [Future] that completes with `true` if operation was successful,
  /// or `false` otherwise.
  static Future<bool> enableAutoPip(VideoPlayerController controller, {int? width, int? height}) {
    if (controller.textureId == VideoPlayerController.kUninitializedTextureId) {
      debugPrint('VideoPlayerPip: Cannot enable auto PiP with uninitialized controller');
      return Future.value(false);
    }
    return _platform.enableAutoPip(controller.textureId, width: width, height: height);
  }

  /// Disables auto-enter Picture-in-Picture mode.
  ///
  /// This prevents the PiP mode from being automatically triggered when the app is backgrounded.
  static Future<bool> disableAutoPip() {
